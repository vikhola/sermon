...
class LoggerProcessor extends LoggerUtil {
    ...
    constructor() {
        super(
            ...
            ["level", level], ["pri", priority], ["pub", publisher], ["timestamp", timestamp], ["app_name", app_name]
        )
    }
...
function app_name(message) {
    if(message.has("app_name") || !this.context?.app_name) return message
    else return message.set("app_name", this.context?.app_name)
}
